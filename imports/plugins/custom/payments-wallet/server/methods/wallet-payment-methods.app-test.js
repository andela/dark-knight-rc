...
import { WalletApi, RISKY_TEST_CARD } from "./walletapi";

const paymentMethod = {
  processor: "Generic",
  storedCard: "Visa 4242",
  paymentPackageId: "vrXutd72c2m7Lenqw",
  paymentSettingsKey: "wallet-paymentmethod",
  status: "captured",
  mode: "authorize",
  createdAt: new Date()
};
...
describe("WalletApi", function () {
  ...
  it("should return data from ThirdPartAPI capture", function (done) {
    ...
    const amount = 19.99;
    const results = WalletApi.methods.capture.call({
...
describe("Submit payment", function () {
  ...
  it("should call Wallet API with card and payment data", function () {
    ...
    const authorizeStub = sandbox.stub(WalletApi.methods.authorize, "call", () => authorizeResult);
    const results = Meteor.call("walletSubmit", "authorize", cardData, paymentData);
  ...
  it("should throw an error if card data is not correct", function () {
    ...
    expect(function () {
      Meteor.call("walletSubmit", "authorize", badCardData, paymentData);
    }
    ).to.throw;
  });
});
...
describe("Capture payment", function () {
  ...
  it("should call WalletApi with transaction ID", function () {
    ...
    const captureStub = sandbox.stub(WalletApi.methods.capture, "call", () => captureResults);
    const results = Meteor.call("wallet/payment/capture", paymentMethod);
  ...
  it("should throw an error if transaction ID is not found", function () {
    sandbox.stub(WalletApi.methods, "capture", function () {
      throw new Meteor.Error("Not Found");
    });
    expect(function () {
      Meteor.call("wallet/payment/capture", "abc123");
    }).to.throw;
  });
});
...
describe("Refund", function () {
  ...
  it("should call WalletApi with transaction ID", function () {
    ...
    const refundStub = sandbox.stub(WalletApi.methods.refund, "call", () => refundResults);
    Meteor.call("wallet/refund/create", paymentMethod, amount);
  ...
  it("should throw an error if transaction ID is not found", function () {
    sandbox.stub(WalletApi.methods.refund, "call", function () {
      throw new Meteor.Error("404", "Not Found");
    });
    const transactionId = "abc1234";
    paymentMethod.transactionId =  transactionId;
    expect(function () {
      Meteor.call("wallet/refund/create", paymentMethod, 19.99);
...
describe("List Refunds", function () {
  ...
  it("should call WalletApi with transaction ID", function () {
    ...
    const refundStub = sandbox.stub(WalletApi.methods.refunds, "call", () => refundResults);
    Meteor.call("wallet/refund/list", paymentMethod);
    expect(refundStub).to.have.been.calledWith(refundArgs);
  });

  it("should throw an error if transaction ID is not found", function () {
    sandbox.stub(WalletApi.methods, "refunds", function () {
      throw new Meteor.Error("404", "Not Found");
    });
    expect(() => Meteor.call("wallet/refund/list", paymentMethod)).to.throw(Meteor.Error, /Not Found/);
  });
});
